...
class XJEPA(nn.Module):
    def __init__(self, in_chans1, in_chans2, patch_size, img_size):
        super().__init__()
        self.ctxencoder1 = vit_base(in_chans=in_chans1, img_size=[img_size], patch_size=patch_size)
        self.ctxencoder2 = vit_base(in_chans=in_chans2, img_size=[img_size], patch_size=patch_size)
        self.tgtencoder1 = copy.deepcopy(self.ctxencoder1)
        self.tgtencoder2 = copy.deepcopy(self.ctxencoder2)
        for param in self.tgtencoder1.parameters():
            param.requires_grad = False
        for param in self.tgtencoder2.parameters():
            param.requires_grad = False
    ...
    def forward(self, images1, images2, masks_enc=None, masks_pred=None):
        if self.training:
            z_ctx1 = self.ctxencoder1(images1, masks_enc)
            z_ctx2 = self.ctxencoder2(images2, masks_enc)
            z_tgt1 = self.tgtencoder1(images1, masks_pred)
            z_tgt2 = self.tgtencoder2(images2, masks_pred)
            ...
            z_emb1 = self.tgtencoder1(images1, full_img_mask)
            z_emb2 = self.tgtencoder2(images2, full_img_mask)

...
from data_loading import LeJEPAStreamingDataset, MultiChannelDataset
...
def main(args):
    ...
    train_dataset = LeJEPAStreamingDataset(metadata=args.metadata, split='train', transform_s1=transform_s1, transform_s2=transform_s2, input_dir='data/opt_BEN_14k/train')
    
    data_loader = StreamingDataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=True, drop_last=True, persistent_workers=True if args.num_workers > 0 else False, prefetch_factor=4 if args.num_workers > 0 else None)
    
    # Prepare validation data
    test_transform_s1 = make_transforms_test(args.in_chans1, 120)
    test_transform_s2 = make_transforms_test(args.in_chans2, 120)
    test_dataset = LeJEPAStreamingDataset(metadata=args.metadata, split='test', transform_s1=test_transform_s1, transform_s2=test_transform_s2, input_dir='data/opt_BEN_14k/test', num_views=1)
    val_dataset = LeJEPAStreamingDataset(metadata=args.metadata, split='validation', transform_s1=test_transform_s1, transform_s2=test_transform_s2, input_dir='data/opt_BEN_14k/validation', num_views=1)
    test_data_loader = StreamingDataLoader(test_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=False, persistent_workers=True if args.num_workers > 0 else False, prefetch_factor=4 if args.num_workers > 0 else None)
    val_data_loader = StreamingDataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=True, drop_last=False, persistent_workers=True if args.num_workers > 0 else False, prefetch_factor=4 if args.num_workers > 0 else None)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for itr, (views_s1, views_s2, labels) in enumerate(data_loader):
            ...
            with autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_amp):
                ...
                sigreg_loss, inv_loss, probe_loss, center_loss = loss_fn(torch.cat([proj1, proj2]), yhat, labels.repeat(2*train_dataset.V, 1))
                lejepa_loss = sigreg_loss * args.lamb + inv_loss * (1 - args.lamb) + center_loss
        ...
        if epoch % args.eval_freq == 0 or epoch == args.epochs:
            model.eval()
            val_embs1, val_embs2, val_labs = [], [], []

            with torch.inference_mode():
                # Build retrieval index from validation set
                for images1, images2, labels in val_data_loader:
                    images1 = images1.squeeze().to(device, non_blocking=True)
                    images2 = images2.squeeze().to(device, non_blocking=True)
                    labels = labels.squeeze().int().to(device, non_blocking=True)
                    with autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_amp):
                        emb1, emb2, _ = model(images1, images2)
                    val_embs1.append(emb1)
                    val_embs2.append(emb2)
                    val_labs.append(labels)
                ...
                # Query with test set
                for images1, images2, q_labels in test_data_loader:
                    images1 = images1.squeeze().to(device, non_blocking=True)
                    images2 = images2.squeeze().to(device, non_blocking=True)
                    q_labels = q_labels.squeeze().int().to(device, non_blocking=True)
                    with autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_amp):
                        emb1, emb2, _ = model(images1, images2)
                    ...
                    for i in range(qhat_s1.shape[0]):
                        ...
                        for mode, topk, ret_mod in [('s1s1', topk_s1s1, 1), ('s2s2', topk_s2s2, 2), 
                            ...
                            for k in range(args.top_k):
                                L_r = val_labs[topk[i, k]]
                                metrics[mode]['prec'] += torch.sum(L_q & L_r) / torch.sum(L_r)
                                metrics[mode]['rec'] += torch.sum(L_q & L_r) / torch.sum(L_q)
                        n += 1
        ...
        if epoch % args.save_freq == 0 or epoch == args.epochs:
            ...
            save_path = os.path.join(args.output_dir, f'{type(model).__name__}_opt_ep{epoch}.pth')
...
# python src_refactored/train_lejepa_opt.py --use_amp --use_asl --num_workers 12 --batch_size 512
...
# B N E (1, 98, 784)
    ...
    probe1 = SharedPredictor(
        num_patches=num_patches,
        embed_dim=embed_dim1,
        predictor_embed_dim=pred_emb_dim,
        depth=pred_depth,
    ).to(device)
    probe2 = SharedPredictor(
        num_patches=num_patches,
        embed_dim=embed_dim1,
        predictor_embed_dim=pred_emb_dim,
        depth=pred_depth,
    ...
    return encoder1, encoder2, probe1, probe2


def load_checkpoint(checkpoint_path, encoder1, encoder2, probe1, probe2, optimizer=None, scheduler=None, device='cpu'):
    """
    ...
    probe1.load_state_dict(checkpoint['probe1'])
    probe2.load_state_dict(checkpoint['probe2'])
...
def train_one_epoch(
    ...
    for itr, ((images1, masks_enc1, masks_pred1), (images2, masks_enc2, masks_pred2)) in enumerate(data_loader):
        ...
        with torch.amp.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=use_amp and device.type != 'mps'):
            ...
            y_1_2 = probe1(z_context1, masks_enc1, masks_pred2, z_context2)
            y_2_1 = probe2(z_context2, masks_enc2, masks_pred1, z_context1)
...
def main(args):
    ...
    # Initialize models
    encoder1, encoder2, probe1, probe2 = init_model(
        device=device,
    ...
    # Create transforms
    transform = make_transforms(num_channels=args.in_chans1 + args.in_chans2) # Check with test transforms as well!
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % args.save_freq == 0 or epoch == args.epochs:
            checkpoint = {
                ...
                'probe1': probe1.state_dict(),
                'probe2': probe2.state_dict(),
